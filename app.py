...
st.markdown("""
    <style>
        /* 1. Kill all default padding and forced centering */
        ...
        /* 2. Force Pydeck to be the fixed background */
        ...
        /* 3. Sidebar glass overlay (Overlays the map) */
        ...
        /* 4. Floating bottom-aligned timeline */
        .stSlider {
            position: fixed !important;
...
for key in ['processed_df', 'radar_cache', 'time_list', 'active_gdf']:
...
# --- 4. THE FULL-SCREEN RENDER ---
if st.session_state.time_list:
    t_idx = st.select_slider("Timeline", options=range(len(st.session_state.time_list)), format_func=lambda x: st.session_state.time_list[x], label_visibility="collapsed")
    ...
    layers = [
        ...
        pdk.Layer("GeoJsonLayer", st.session_state.active_gdf.__geo_interface__, stroked=True, filled=False, get_line_color=[255, 255, 255], line_width_min_pixels=3)
    ]
else:
    layers = [pdk.Layer("GeoJsonLayer", st.session_state.active_gdf.__geo_interface__, stroked=True, filled=False, get_line_color=[255, 255, 255], line_width_min_pixels=3)] if st.session_state.active_gdf is not None else []

st.pydeck_chart(pdk.Deck(layers=layers, initial_view_state=st.session_state.map_view, map_style="dark"), key="radar_map")
